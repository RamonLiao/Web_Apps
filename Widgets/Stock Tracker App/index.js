...
const apiKey = "WDO4D1WYF7VQXS8V"; // or 'demo'

// Function to fetch and display the top 10 stocks
function fetchTopStocks() {
  // Fetch data from api
  let url = `https://www.alphavantage.co/query?function=SECTOR&apikey=${apiKey}`;
  fetch(url)
    .then((response) => response.json())
    .then((data) => {
      const stocks = data["Rank A: Real-Time Performance"]; // it is an object not a list
      const stocksLength = Object.keys(stocks).length;
      let html = "";
...
function fetchStockData(symbol) {
  // If input was empty, then display top 10 stocks
  if (!symbol) {
    fetchTopStocks();
    return;
  }
