class GenNumber extends React.Component {
  componentDidUpdate() {
    ...
    setTimeout(function () {
      ...
      /** g -> global match; i -> case intensive match */
      number.innerHTML = number.innerHTML.replace(/\w/gi, "&#183;"); // "·" Interpunct = "&#183;" (Numeric Entity)
    }, time);
  }
  ...
  componentDidMount() {
    let number = document.getElementById("number");
    setTimeout(function () {
      number.innerHTML = number.innerHTML.replace(/\w|\W/gi, "&#183;"); // "·" Interpunct = "&#183;" (Numeric Entity)
    }, 1200);
  }
...
class InputNumber extends React.Component {
  constructor() {
    super();
    // Bind event handlesrs to the componenet instance
  ...
  handleUserInput(e) {
    ...
    // Pass the user input to thye parent component for comparison
    this.props.compareUserInput(userNumber);
  }
  handleReset() {
    // Call the reset fun;ction provided by parent component
    this.props.onReset();
  }
...
class App extends React.Component {
  constructor() {
    super();
    // Bind event handlers to the components instance
...
// Render maincomponent to the HTML element with id "app"
